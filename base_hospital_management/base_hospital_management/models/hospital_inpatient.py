...
class HospitalInpatient(models.Model):
        ...
        for rec in self:
            if rec.hosp_date:
                if rec.discharge_date:
                    rec.admit_days = (rec.discharge_date - rec.hosp_date +
                                       datetime.timedelta(days=1)).days
                else:
                    rec.admit_days = (fields.Date.today() - rec.hosp_date +
                                       datetime.timedelta(days=1)).days
            else:
                rec.admit_days = 0
    ...
    def _compute_room_rent_amount(self):
        ...
        for rec in self:
            if rec.hosp_date:
                if rec.discharge_date:
                    days = (rec.discharge_date - rec.hosp_date + datetime.timedelta(days=1)).days
                else:
                    days = (fields.Date.today() - rec.hosp_date + datetime.timedelta(days=1)).days
                rec.room_rent_amount = (rec.room_id.rent or 0) * days
            else:
                rec.room_rent_amount = 0
        ...
        for rec in self:
            if rec.hosp_date:
                if rec.discharge_date:
                    ...
                    days = (fields.Date.today() - rec.hosp_date + datetime.timedelta(days=1)).days
                rec.bed_rent_amount = (rec.bed_id.bed_rent if rec.bed_id else 0) * days
            else:
                rec.bed_rent_amount = (rec.bed_id.bed_rent if rec.bed_id else 0)
